...
-- 128 registros es uno que alguien miró.
--
-- Tres decisiones suyas sobre casos que le marqué aparte:
--   · IPL y LUZ PULSADA INTENSA eran el mismo tratamiento cargado dos veces con
--     el mismo precio. Quedan las de IPL como servicio; las de LUZ PULSADA se
--     borran como duplicados y NO se rearman como combos. Se suma acá
--     `LUZ PULSADA INTENSA - Sesion Facial` ($62.000), que NO estaba en la
--     planilla —su nombre no disparaba ningún filtro— pero es duplicado exacto
--     de `IPL Rostro - Sesión` ($62.000). Sobrevive a propósito
--     `LUZ PULSADA INTENSA - En MANOS // AXILAS` ($53.000): no tiene par en IPL.
--   · Las de `Radiofrecuencia Facial` quedan como servicio: son limpieza facial
--     con radiofrecuencia, no un paquete de zonas.
...
--
-- Verificado contra producción: los 64 servicios tienen CERO turnos y CERO
-- líneas de factura, así que no hay historial que proteger.
--
-- OJO: `promotions` queda VACÍA. Las 65 filas eran todas `promotion_type='bundle'`
-- sin importe: combos duplicados, no promociones con descuento. La pestaña Promos
-- y la sección de la web quedan vacías hasta que Laura cargue promos reales.
--
-- Solo `service_embeddings` tiene ON DELETE CASCADE; el resto de las hijas es
-- NO ACTION, así que se borran explícitamente y en orden.

BEGIN;

CREATE TEMP TABLE _serv_borrar (id uuid PRIMARY KEY) ON COMMIT DROP;
INSERT INTO _serv_borrar (id) VALUES
    ...
    ('c7cf8512-bad0-4aed-9b96-a256e8d3d04e'),  -- LUZ PULSADA INTENSA -  Sesión Facial + cuello, escote y mano
    ...
    ('8ddb1d2c-7c3e-497b-abcc-8a751dd05846'),  -- Espalda completa + Hombros + Brazos + Pecho + Abdomen (axila
    ...
    ('812e4209-f63d-4c0c-918a-c1d532737575'),  -- Pierna entera, Abdomen, Brazos, Pelvis, Tira de cola y Axila
    ...
    ('fae5a49e-bb95-4c4f-a1c1-ff2067e7be58'),  -- Pierna entera, Cavado completo, Rostro completo y Axila o Ti
    ...
    ('3205d84f-5f3c-4fad-bdb4-453701c4cf2b'),  -- Pierna entera, cavado, tira de cola, rostro completo, axilas
    ...
    ('1ce567be-1a32-4261-86b6-0d62061d2b3c'),  -- Pierna entera, Rostro completo, Axila, Cavado y Tira de cola
    ...
    ('46b44883-14d6-4528-bcbd-2bbc1d030133'),  -- Rostro, Cavado, Axilas, Tira de cola, Brazos y Espalda baja
    ('32e8dc86-7e5d-4cae-846e-dbea9d4a259c'),  -- Vela Slim Plus/Max - Combo Zona Superior / Piernas completas
    ('9a1fea52-abf0-46f4-baef-c42e9826fcbf'),  -- Venus Legacy - Combo Zona Superior / Piernas completas
    ('5a8f65ff-6a71-4941-abc2-72ab021d9753')  -- LUZ PULSADA INTENSA - Sesion Facial (duplicado exacto de IPL Rostro)
;

CREATE TEMP TABLE _promo_borrar (id uuid PRIMARY KEY) ON COMMIT DROP;
INSERT INTO _promo_borrar (id) VALUES
    ...
    ('a462ec99-43f3-4988-a00b-ca66c66e536f'),  -- Crio-Radiofrecuencia - Combo Zona Superior / Piernas complet
    ...
    ('fb7b4476-0627-4695-aca3-f9438588f314'),  -- Espalda completa + Hombros + Brazos + Pecho + Abdomen (axila
    ...
    ('be1db2b7-fd95-47b9-805b-55dc6e82b150'),  -- Pierna entera, Abdomen, Brazos, Pelvis, Tira de cola y Axila
    ...
    ('25bfbeb5-58d4-4e19-938c-87c0b9376f37'),  -- Pierna entera, Cavado completo, Rostro completo y Axila o Ti
    ...
    ('b066d999-e7dc-43ff-8e2b-789596977c47'),  -- Pierna entera, cavado, tira de cola, rostro completo, axilas
    ...
    ('32c4e4ec-4f95-497d-9fd0-89f9709c9c51'),  -- Pierna entera, Rostro completo, Axila, Cavado y Tira de cola
    ...
    ('86ff2c21-1abe-4f4d-872e-672d80abb088'),  -- Rostro, Cavado, Axilas, Tira de cola, Brazos y Espalda baja
    ('1031057a-2654-4a3f-94fc-c7fb3536aa8f'),  -- Vela Slim Plus/Max - Combo Zona Inferior
    ('4c3a2d2a-f95e-47bf-95e7-9e55b3bf5c7a'),  -- Vela Slim Plus/Max - Combo Zona Superior / Piernas completas
...
\echo '── ANTES ──'
SELECT (SELECT count(*) FROM service WHERE is_active) AS servicios_activos,
       (SELECT count(*) FROM promotions)              AS promociones,
       (SELECT count(*) FROM promotions p WHERE NOT EXISTS (
          SELECT 1 FROM promotion_service ps JOIN service s ON s.id = ps.service_id
          WHERE ps.promotion_id = p.id AND s.is_active))     AS promos_vacias;

-- Guarda: si alguno tuviera historial, se planta antes de tocar nada.
DO $$
DECLARE n int;
BEGIN
  SELECT count(*) INTO n FROM appointments a JOIN _serv_borrar b ON a.service_id = b.id;
  IF n > 0 THEN RAISE EXCEPTION 'ABORTA: % turnos apuntan a servicios a borrar', n; END IF;
  SELECT count(*) INTO n FROM line_items li JOIN _serv_borrar b ON li.service_id = b.id;
  IF n > 0 THEN RAISE EXCEPTION 'ABORTA: % lineas de factura apuntan a servicios a borrar', n; END IF;
  SELECT count(*) INTO n FROM combo_service cs JOIN _serv_borrar b ON cs.service_id = b.id;
  IF n > 0 THEN RAISE EXCEPTION 'ABORTA: % lineas de combo apuntan a servicios a borrar', n; END IF;
END $$;

-- 1. Hijas de promotions
DELETE FROM promotion_service  WHERE promotion_id IN (SELECT id FROM _promo_borrar);
DELETE FROM promotion_product  WHERE promotion_id IN (SELECT id FROM _promo_borrar);

-- 2. Hijas de service. `promotion_service` aparece otra vez por si una promoción
--    que se queda apuntara a un servicio que se va; hoy son 0, pero no se asume.
DELETE FROM promotion_service        WHERE service_id IN (SELECT id FROM _serv_borrar);
DELETE FROM service_category         WHERE service_id IN (SELECT id FROM _serv_borrar);
DELETE FROM service_machine          WHERE service_id IN (SELECT id FROM _serv_borrar);
DELETE FROM service_provider_service WHERE service_id IN (SELECT id FROM _serv_borrar);

-- 3. Los registros. `service_embeddings` cae solo (CASCADE).
DELETE FROM promotions WHERE id IN (SELECT id FROM _promo_borrar);
DELETE FROM service    WHERE id IN (SELECT id FROM _serv_borrar);
...
\echo '── huérfanos (las cinco deben dar 0) ──'
SELECT (SELECT count(*) FROM promotion_service ps
          WHERE NOT EXISTS (SELECT 1 FROM service s WHERE s.id = ps.service_id))    AS ps_sin_servicio,
       (SELECT count(*) FROM service_category sc
          WHERE NOT EXISTS (SELECT 1 FROM service s WHERE s.id = sc.service_id))    AS cat_sin_servicio,
       (SELECT count(*) FROM service_embeddings se
          WHERE NOT EXISTS (SELECT 1 FROM service s WHERE s.id = se.service_id))    AS emb_sin_servicio,
       (SELECT count(*) FROM service_machine sm
          WHERE NOT EXISTS (SELECT 1 FROM service s WHERE s.id = sm.service_id))    AS maq_sin_servicio,
       (SELECT count(*) FROM service_provider_service sp
          WHERE NOT EXISTS (SELECT 1 FROM service s WHERE s.id = sp.service_id))    AS prov_sin_servicio;

\echo '── control: lo que Laura pidió conservar sigue vivo, y los duplicados no ──'
SELECT
  (SELECT count(*) FROM service WHERE is_active AND name IN (
     'Radiofrecuencia Facial - Rostro y Papada',
     'Radiofrecuencia Facial - Rostro, Cuello y Escote',
     'Radiofrecuencia Fraccionada - Cuello y Escote o Contorno Ocular - Sesión',
     'Radiofrecuencia Fraccionada Facial con Papada, Cuello y Escote - Sesión',
     'Retiro de esmalte semi de manos y pies realizado en otro espacio (extra)'
  ))                                                                    AS rescatados_deben_ser_5,
  (SELECT count(*) FROM service WHERE name ILIKE 'IPL %')               AS ipl_quedan_3,
  (SELECT count(*) FROM service WHERE name ILIKE '%LUZ PULSADA%')       AS luz_pulsada_debe_quedar_1_manos_axilas;
