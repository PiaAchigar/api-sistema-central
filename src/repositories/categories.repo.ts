...
/** Lista categorías ordenadas. `includeInactive` suma las archivadas (solo admin/staff). */
export async function listCategories(db: Db, includeInactive = false) {
  const base = db.select(categoryFields).from(categories);
  const ordered = includeInactive ? base : base.where(eq(categories.isActive, true));
  return ordered.orderBy(asc(categories.displayOrder), asc(categories.name));
}
